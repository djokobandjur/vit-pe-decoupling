...
# -*- coding: utf-8 -*-
"""
Cross-family functional robustness probe for ImageNet-100 ViT-S/16 positional encodings.

Purpose
-------
Evaluate stochastic (random coherent) and adversarial (PGD) perturbations of
Learned, Sinusoidal, RoPE, and ALiBi positional mechanisms on the *new
...
MODEL_KWARGS_BASE = dict(embed_dim=384, depth=12, num_heads=6, mlp_ratio=4.0, dropout=0.0)
...
def sha256_json_object_local(obj: Mapping) -> str:
    raw=json.dumps(obj,sort_keys=True,separators=(",",":"),ensure_ascii=False).encode("utf-8")
    return hashlib.sha256(raw).hexdigest()


def load_frozen_split(path: Path, dataset, val_dir: Path):
    path=path.expanduser().resolve()
    payload=json.loads(path.read_text(encoding="utf-8"))
    stored=payload.get("split_sha256")
    computed=sha256_json_object_local({k:v for k,v in payload.items() if k!="split_sha256"})
    if not stored or stored!=computed:
        raise RuntimeError(f"Frozen split self-hash mismatch: stored={stored}, computed={computed}")
    cal=list(map(int,payload["calibration_indices"])); attack=list(map(int,payload["attack_indices"])); ev=list(map(int,payload["evaluation_indices"]))
    all_idx=cal+attack+ev; n_total=len(dataset)
    if len(all_idx)!=n_total or len(set(all_idx))!=n_total or min(all_idx)<0 or max(all_idx)>=n_total:
        raise RuntimeError("Frozen split is not a unique full partition of the current dataset")
    if set(cal)&set(attack) or set(cal)&set(ev) or set(attack)&set(ev):
        raise RuntimeError("Frozen split partitions overlap")
    if [len(cal),len(attack),len(ev)] != [int(payload["counts"]["calibration"]),int(payload["counts"]["attack"]),int(payload["counts"]["evaluation"])]:
        raise RuntimeError("Frozen split counts mismatch")
    records=[]
    classes=list(dataset.classes)
    for sample_path,target in dataset.samples:
        records.append({"relative_path":str(Path(sample_path).resolve().relative_to(val_dir.resolve())),"class_index":int(target),"class_name":classes[int(target)]})
    sample_hash=sha256_json_object_local(records)
    if sample_hash!=payload["sample_order_sha256"]:
        raise RuntimeError(f"Validation sample-order hash mismatch: current={sample_hash}, frozen={payload['sample_order_sha256']}")
    return payload,cal,attack,ev,sha256_file_local(path)


def load_checkpoint_manifest(path: Path):
    path=path.expanduser().resolve(); payload=json.loads(path.read_text(encoding="utf-8"))
    if not payload.get("complete"):
        raise RuntimeError("Checkpoint manifest is not complete")
    rows={(str(r["family"]),int(r["seed"])):r for r in payload.get("rows",[])}
    if len(rows)!=24:
        raise RuntimeError(f"Expected 24 unique checkpoint rows, found {len(rows)}")
    return payload,rows,sha256_file_local(path)


def checkpoint_from_manifest(rows, models_dir: Path, pe_type: str, seed: int) -> Path:
    key=(pe_type,int(seed))
    if key not in rows: raise KeyError(f"Checkpoint not present in frozen manifest: {key}")
    row=rows[key]; checkpoint=Path(row["checkpoint"]).expanduser().resolve()
    canonical=(models_dir/f"{pe_type}_seed{seed}"/"best_model.pth").resolve()
    if checkpoint!=canonical:
        raise RuntimeError(f"Manifest checkpoint is not canonical best_model.pth: {checkpoint} != {canonical}")
    if not checkpoint.is_file(): raise FileNotFoundError(checkpoint)
    current=sha256_file_local(checkpoint)
    if current!=row.get("sha256"):
        raise RuntimeError(f"Checkpoint SHA-256 mismatch for {pe_type} seed {seed}: current={current}, frozen={row.get('sha256')}")
    if not row.get("strict_load_ok"):
        raise RuntimeError(f"Checkpoint was not strict-valid in manifest: {pe_type} seed {seed}")
    return checkpoint
...
def load_or_create_results(args: argparse.Namespace, n_total: int, split_info: Mapping) -> MutableMapping:
    ...
    if output_path.exists() and not args.overwrite:
        ...
        if meta.get("split_file_sha256") != args.split_file_sha256:
            raise RuntimeError("Resume refused: frozen split file hash differs from existing output")
        if meta.get("checkpoint_manifest_sha256") != args.checkpoint_manifest_sha256:
            raise RuntimeError("Resume refused: checkpoint manifest hash differs from existing output")
        return payload
    ...
    return {
        "metadata": {
            ...
            "split_file_sha256": args.split_file_sha256,
            "checkpoint_manifest": str(Path(args.checkpoint_manifest).resolve()),
            "checkpoint_manifest_sha256": args.checkpoint_manifest_sha256,
            "checkpoint_filename_policy": "best_model.pth only; hash-verified against strict validator manifest",
            ...
            "config": {
                ...
                "architecture": {"name": "ViT-S/16", "embed_dim": 384, "depth": 12, "num_heads": 6, "head_dim": 64, "num_tokens": 197},
            },
            "rho_definition": {
...
def run(args: argparse.Namespace) -> None:
    ...
    n_total = len(dataset)
    split_payload,cal_idx,attack_idx,eval_idx,args.split_file_sha256 = load_frozen_split(
        Path(args.split_file), dataset, Path(args.val_dir)
    )
    seed_everything(int(split_payload["split_seed"]))
    args.checkpoint_manifest_payload,args.checkpoint_rows,args.checkpoint_manifest_sha256 = load_checkpoint_manifest(Path(args.checkpoint_manifest))
    split_info = {
        "seed": int(split_payload["split_seed"]),
        ...
        "sample_order_sha256": split_payload["sample_order_sha256"],
    }
    ...
    for pe_type in args.pe_types:
        ...
        for training_seed in args.seeds:
            ...
            checkpoint = checkpoint_from_manifest(args.checkpoint_rows, Path(args.models_dir).resolve(), pe_type, training_seed)
            record = ensure_run_record(results, pe_type, training_seed, checkpoint)
            record["checkpoint_sha256"] = sha256_file_local(checkpoint)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--split-file", required=True, help="Frozen imagenet100_split_indices.json; loaded and hash-verified")
    parser.add_argument("--checkpoint-manifest", required=True, help="Strict validator manifest; exact checkpoint paths and hashes are enforced")
