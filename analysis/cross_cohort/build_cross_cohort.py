...
def write_tables(out,report,agg,contrast):
    ...
    lines=[
        ...
        '\\centering',
        '\\small',
        f'\\caption{{Cross-architecture ImageNet-100 normalized AUC over the locked common-support interval $\\rho_{{\\mathrm{{rel}}}}\\in[0,{end:.5f}]$. Values are mean $\\pm$ sample SD across the same six training seeds. Objective-specific audit points are excluded from this canonical support.}}',
